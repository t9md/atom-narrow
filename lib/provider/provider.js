...
class Provider {
  ...
  buildSearchOptionsProps ({queryCurrentWord}) {
    ...
    return {
      searchWholeWord:
        (queryCurrentWord && editor.getSelectedBufferRange().isEmpty()) || this.getConfig('searchWholeWord'),
  ...
  getState () {
    if (this.searchOptions) {
      return {searchOptionState: this.searchOptions.getState()}
    } else {
      return {}
    }
  }
  ...
  saveState () {
    const provdierState = Object.assign(this.getState(), this.callHook('willSaveState'))
    return {
      name: this.dashName,
      options: {query: this.ui.lastQuery},
      state: {
        provider: provdierState,
        ui: this.ui.getState()
      }
    }
  }
