...
module.exports = class Symbols extends ProviderBase {
  constructor(...args) {
    super(...args)
  ...
  itemForTag({position, name}) {
    return {
      point: getFirstCharacterPositionForBufferRow(this.editor, position.row),
      text: "  ".repeat(this.getLevelForRow(position.row)) + name,
    }
  }
