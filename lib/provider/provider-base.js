...
class ProviderBase {
  ...
  async start() {
    ...
    if (ready) {
      ...
      await this.ui.open({pending, focus})
      return this.ui
    }
  }
  ...
  async openFileForItem({filePath}, {activatePane} = {}) {
    ...
    if (WorkspaceOpenAcceptPaneOption) {
      ...
      pane.activate()
      const editor = await atom.workspace.open(filePath, openOptions)
      if (!activatePane) originalActivePane.activate()
      return editor
    }
  }
  ...
  async confirmed(item) {
    ...
    const editor = await this.openFileForItem(item, {activatePane: true})
  ...
  updateSearchState() {
    ...
    this.ui.controlBar.updateElements({
      searchRegex: this.searchOptions.searchRegex,
      searchWholeWord: this.searchOptions.searchWholeWord,
      searchIgnoreCase: this.searchOptions.searchIgnoreCase,
      searchTerm: this.searchOptions.searchTerm,
      searchUseRege: this.searchOptions.searchUseRege,
    })
  }
  ...
  async scanItemsForFilePath(filePath, regex) {
    const editor = await atom.workspace.open(filePath, {activateItem: false})
