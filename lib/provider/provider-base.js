...
class ProviderBase {
  ...
  getFilterSpec(filterQuery) {
    if (filterQuery) {
      const negateByEndingExclamation = this.getConfig(
  ...
  updateSearchState() {
    ...
    if (this.searchOptions.grammarCanHighlight) {
      this.ui.grammar.setSearchRegex(this.searchOptions.searchRegex)
    } else {
      this.ui.grammar.setSearchRegex(null)
    }
  ...
  scanItemsForBuffer(buffer, regExp) {
    ...
    for (let row = 0; row < lines.length; row++) {
      ...
      while ((match = regExp.exec(lineText))) {
        const point = [row, match.index]
        const column = match.index
        const range = new Range([row, column], [row, column + match[0].length])
        items.push({text: lineText, point: range.start, range, filePath})
  ...
  scanItemsForFilePath(filePath, regExp) {
    return atom.workspace.open(filePath, {activateItem: false}).then(editor => {
      return this.scanItemsForBuffer(editor.buffer, regExp)
    })
  }
}
