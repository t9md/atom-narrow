...
module.exports = class ProviderBase {
  ...
  static start (name, options = {}, state) {
    if (!(name in this.providersByName)) {
      const filePath = this.providerPathsByName[name] || `./${_.dasherize(name)}`
      this.providersByName[name] = require(filePath)
    }

    const editor = atom.workspace.getActiveTextEditor()
    const Klass = this.providersByName[name]
  ...
  initializeSearchOptions (restoredState) {
    ...
    if (!restoredState) {
      const queryWordWithoutSelection = this.options.queryCurrentWord && editor.getSelectedBufferRange().isEmpty()
      initialState = {
