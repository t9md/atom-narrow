...
class Search extends ProviderBase {
  ...
  searchFilePath(filePath) {
    ...
    return this.scanItemsForFilePath(filePath, searchRegex).then(items =>
  ...
  async getItems({filePath}) {
    ...
    if (filePath) {
      const items = atom.project.contains(filePath)
        ? await this.scanItemsForFilePath(filePath, searchRegex)
        : []
      this.finishUpdateItems(items)
    } else {
      this.search()
