...
let SelectFiles
let lastLocationByProviderName = {}
...
class Ui {
  ...
  updateLastLocation () {
    lastLocationByProviderName[this.provider.name] = this.narrowEditor.getLocation()
  }

  getLocationToOpen () {
    return lastLocationByProviderName[this.provider.name] || this.locationToOpen
  }

  async open ({focus = true} = {}) {
    const location = this.getLocationToOpen()
    if (location === 'center') {
