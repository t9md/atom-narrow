...
module.exports = class Ui {
  ...
  constructor (provider, {query = ''} = {}, restoredState) {
    if (!SelectFiles) {
      SelectFiles = require('./composition-provider/select-files')
    }
  ...
  createStateToReduce () {
    return {
      ...
      allItems: [],
      filterSpec: this.getFilterSpec(this.provider.name, this.getFilterQuery()),
      filterSpecForSelectFiles: this.getFilterSpec('SelectFiles', this.queryForSelectFiles),
      fileExcluded: false,
      excludedFiles: this.excludedFiles,
  ...
  getFilterSpec (providerName, filterQuery) {
    if (filterQuery) {
      return new FilterSpec(filterQuery, {
        negateByEndingExclamation: settings.get(`${providerName}.negateNarrowQueryByEndingExclamation`),
        sensitivity: settings.get(`${providerName}.caseSensitivityForNarrowQuery`)
      })
    }
  }
