...
class Ui {
  ...
  constructor(provider, {query} = {}, restoredState) {
    ...
    this.editorElement.onDidChangeScrollTop(() => {
      this.highlightVisibleItems(this.filterSpec)
    })
  ...
  highlightVisibleItems(filterSpec) {
    // TODO
    // - clear markers out of screen
    // - highlight newly became visible items only

    this.highlighter.clearNarrowEditorHighlight()

    let [startRow, endRow] = this.editorElement.getVisibleRowRange()
    startRow = this.editor.bufferRowForScreenRow(startRow)
    endRow = this.editor.bufferRowForScreenRow(endRow)

    const items = this.items.items.slice(startRow, endRow + 1)
    this.highlighter.highlightNarrowEditorForItems(items, filterSpec)
  }
  ...
  renderItems({renderStartPosition, items, filterSpec}) {
    ...
    const visibleItemCount = this.editor.getRowsPerPage() - renderStartPosition.row
    ...
    if (visibleItemCount > 0) {
      ...
      this.highlighter.highlightNarrowEditorForItems(visibleItems, filterSpec)
    }
