...
const ItemEditor = require('./item-editor')
...
class Ui {
  ...
  registerCommands () {
    return atom.commands.add(this.editorElement, {
      ...
      'core:move-up': event => this.itemEditor.moveUpOrDownWrap(event, 'up'),
      'core:move-down': event => this.itemEditor.moveUpOrDownWrap(event, 'down'),
      ...
      'vim-mode-plus:move-up-wrap': () => this.itemEditor.preserveGoalColumn(),
      'vim-mode-plus:move-down-wrap': () => this.itemEditor.preserveGoalColumn(),
      ...
      'narrow-ui:move-to-prompt-or-selected-item': () => this.itemEditor.moveToPromptOrSelectedItem(),
      'narrow-ui:move-to-prompt': () => this.moveToPrompt(),
      'narrow-ui:start-insert': () => this.itemEditor.setReadOnly(false),
      'narrow-ui:stop-insert': () => this.itemEditor.setReadOnly(true),
      ...
      'narrow-ui:move-to-next-file-item': () => this.itemEditor.moveToDifferentFileItem('next'),
      'narrow-ui:move-to-previous-file-item': () => this.itemEditor.moveToDifferentFileItem('previous'),
      'narrow-ui:toggle-search-whole-word': () => this.toggleSearchOptionsAndRefresh('searchWholeWord'),
      'narrow-ui:toggle-search-ignore-case': () => this.toggleSearchOptionsAndRefresh('searchIgnoreCase'),
      'narrow-ui:toggle-search-use-regex': () => this.toggleSearchOptionsAndRefresh('searchUseRegex'),
      'narrow-ui:delete-to-end-of-search-term': () => this.itemEditor.deleteToEndOfSearchTerm(),
  ...
  setQueryAndRefreshWithDelay (text, delay) {
    this.itemEditor.withIgnoreChange(() => this.setQuery(text))
    this.refreshWithDelay({force: true}, delay, () => {
      ...
      this.itemEditor.flashCurrentRow()
    })
  }
  ...
  refreshPromptHighlight () {
    this.highlighter.highlightPrompt(this.itemEditor.parsePromptLine())
  }

  toggleSearchOptionsAndRefresh (name) {
  ...
  toggleProtected () {
    ...
    this.itemEditor.updateItemIndicator({protected: this.protected})
  ...
  constructor (provider, {query = ''} = {}, restoredState) {
    ...
    this.itemEditor = new ItemEditor(this, this.titleNumber)

    this.editor = this.itemEditor.editor
    ...
    this.reducers = [
      ...
      this.itemEditor.renderItems.bind(this.itemEditor)
    ].filter(v => v)
  ...
  async open ({pending = false, focus = true, pane = null} = {}) {
    ...
    this.itemEditor.initialize()

    if (focus && this.provider.needActivateOnStart()) {
      pane.activate()
    }

    this.itemEditor.withIgnoreChange(() => this.setQuery(this.query))
    ...
    if (this.provider.needRevealOnStart()) {
      ...
      if (this.items.hasSelectedItem()) {
        ...
        if (previewd) this.itemEditor.flashCurrentRow()
      }
  ...
  observeItemEditor () {
    return new CompositeDisposable(
      this.itemEditor.onDidDestroyPromptSelection(() => {
        this.controlBar.show()
        this.itemEditor.withIgnoreChange(() => this.setQuery(this.lastQuery)) // Recover query
      }),
      this.itemEditor.onDidChangeQuery(() => {
        ...
        const delay =
          this.useFirstQueryAsSearchTerm && this.itemEditor.getSearchTerm() !== this.lastSearchTerm
  ...
  getPane () { return paneForItem(this.editor) } // prettier-ignore
  isSamePaneItem (item) { return paneForItem(item) === this.getPane() } // prettier-ignore
  isActive () { return this.itemEditor.isActive() } // prettier-ignore
  ...
  focusPrompt () {
    if (this.isActive() && this.itemEditor.isAtPrompt()) {
  ...
  close () {
    ...
    // 2nd arg is force to skip prompted to save if modified
  ...
  getQuery () {
    return this.itemEditor.getQuery()
  }

  setQuery (text) {
    this.itemEditor.setQuery(text)
    this.refreshPromptHighlight()
  }
  ...
  excludeFile () {
    ...
    if (filePath && !this.excludedFiles.includes(filePath)) {
      ...
      this.itemEditor.moveToDifferentFileItem('next')
      this.refresh()
    }
  }
  ...
  async refresh ({force, selectFirstItem, event = {}} = {}) {
    ...
    let searchTerm
    if (this.useFirstQueryAsSearchTerm) {
      searchTerm = this.itemEditor.getSearchTerm()
    ...
    const onFinish = () => {
      ...
      if (!selectFirstItem && oldSelectedItem) {
        ...
        if (!this.itemEditor.isAtPrompt()) {
          this.itemEditor.moveToItem(this.items.getSelectedItem(), oldColumn)
        }
      } else {
        // when originally selected item cannot be selected because of excluded.
        this.items.selectFirstNormalItem()
        if (!this.itemEditor.isAtPrompt()) this.moveToPrompt()
      }
  ...
  syncToEditor (editor) {
    ...
    if (item) {
      this.itemEditor.moveToItem(item)
  ...
  moveToSearchedWordOrBeginningOfSelectedItem () {
    ...
    if (searchOptions && searchOptions.searchRegex) {
      ...
      this.itemEditor.moveToBeginningOfSelectedItem()
    }
  }
