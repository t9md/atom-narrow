...
module.exports = class Ui {
  ...
  previewItemForDirection (direction) {
    ...
    const itemToSelect = firstTimeNext ? selectedItem : this.items.findNormalItem(selectedItem._uiRow, {direction})
    if (itemToSelect) {
  ...
  observeChange () {
    const isAtPrompt = range => range.intersectsWith(this.getPromptRange())
    const isQueryModified = (newRange, oldRange) =>
      (!newRange.isEmpty() && isAtPrompt(newRange)) || (!oldRange.isEmpty() && isAtPrompt(oldRange))

    const destroyPromptSelection = () => {
      const promptSelections = this.editor.getSelections().filter(selection => isAtPrompt(selection.getBufferRange()))
      promptSelections.forEach(selection => selection.destroy())
      if (promptSelections.length) this.controlBar.show()
  ...
  observeCursorMove () {
    return this.editor.onDidChangeCursorPosition(event => {
      ...
      const rowToSelect = itemToSelect._uiRow
