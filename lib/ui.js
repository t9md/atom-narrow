...
class Ui {
  ...
  async queryCurrentWord () {
    ...
    this.itemEditor.itemEditor.withIgnoreChange(() => this.setQuery(word))
  ...
  observeItemEditor () {
    ...
    disposables.add(this.itemEditor.observeChange())
    disposables.add(this.itemEditor.observeCursorMove())
  ...
  moveUpOrDownWrap (event, direction) {
    this.itemEditor.preserveGoalColumn()
    ...
    const onFinish = () => {
      ...
      if (!selectFirstItem && oldSelectedItem) {
        ...
        if (!this.itemEditor.isAtPrompt()) {
          this.itemEditor.moveToSelectedItem({ignoreCursorMove: !this.isActive(), column: oldColumn})
        }
      } else {
  ...
  previewWithDelay () {
    ...
    this.delayedPreviewTimeout = setTimeout(preview, this.workspaceHasSelectedItem() ? 0 : 20)
  }
  ...
  syncToEditor (editor) {
    ...
    if (item) {
      this.itemEditor.moveToItem(item, {ignoreCursorMove: false})
