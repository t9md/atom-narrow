...
class Ui {
  ...
  refreshPromptHighlight () {
    this.highlighter.highlightPrompt(this.narrowEditor.parsePromptLine())
  }
  ...
  constructor (provider, {query = ''} = {}, restoredState) {
    ...
    this.narrowEditor = new NarrowEditor(this, this.titleNumber)
    ...
    this.reducers = [
      ...
      this.narrowEditor.renderItems.bind(this.narrowEditor)
    ].filter(v => v)
  ...
  async open ({pending = false, focus = true, pane = null} = {}) {
    ...
    if (this.provider.needRevealOnStart()) {
      ...
      if (this.items.hasSelectedItem()) {
        ...
        if (previewd) this.narrowEditor.flashCurrentRow()
      }
  ...
  async refresh ({force, selectFirstItem, event = {}} = {}) {
    ...
    const onFinish = () => {
      ...
      if (!selectFirstItem && oldSelectedItem) {
        ...
        this.items.selectFirstNormalItem()
        if (!this.narrowEditor.isAtPrompt()) this.moveToPrompt()
      }
