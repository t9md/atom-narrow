...
class Ui {
  ...
  static setLastFocusedUi (ui) { this.lastFocusedUi = ui } // prettier-ignore
  static getLastFocusedUi () { return this.lastFocusedUi } // prettier-ignore
  ...
  async open ({focus = true} = {}) {
    ...
    // When we focused narrowEditor initially, ignore very first onDidStopChangingActivePaneItem event
    // Since we are doing manual previewing on startup by refresh().
    this.disposables.add(this.observeOnDidStopChangingActivePaneItem({ignoreFirstEvent: focus}))
    this.moveToPrompt()
  ...
  observeOnDidStopChangingActivePaneItem ({ignoreFirstEvent} = {}) {
    return atom.workspace.onDidStopChangingActivePaneItem(item => {
      ...
      if (Ui.has(item)) {
        if (item === this.editor) {
          Ui.setLastFocusedUi(this)
          if (!this.narrowEditor.isAtPrompt() && this.autoPreview) {
            this.previewWithDelay()
          }
        }
      } else {
        this.provider.needRestoreEditorState = false
        if (paneForItem(item) !== this.getPane()) {
          this.startSyncToEditor(item)
        }
        this.highlighter.clearCurrentAndLineMarker()
        this.highlighter.highlightEditor(item)
      }
    })
  }
