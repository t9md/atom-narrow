...
class Ui {
  ...
  getSearchTermFromQuery() {
    if (this.useFirstQueryAsSearchTerm) {
      return this.getQuery().split(/\s+/)[0]
    }
  }
  ...
  deleteToEndOfSearchTerm() {
    ...
    if (!searchTerm) {
      ...
      const cursorColumn = selection.cursor.getBufferColumn()
      const endOfSearchTerm = searchTerm.length
      const startColumn = cursorColumn > endOfSearchTerm ? endOfSearchTerm : 0

      selection.setBufferRange([[0, startColumn], [0, cursorColumn]])
      selection.delete()
    }
  }
  ...
  constructor(provider, {query} = {}, restoredState) {
    ...
    this.grammar = new Grammar(this.editor, {includeHeaderRules: !this.boundToSingleFile})
  ...
  refresh({force, selectFirstItem, event = {}} = {}) {
    ...
    const onUpdate = items => {
      if (!grammarUpdated) {
        ...
        this.grammar.update(state.filterSpec ? state.filterSpec.include : undefined)
        grammarUpdated = true
      }
      this.reduceItems(items, state)
    }
    ...
    this.refreshDisposables.add(this.onDidUpdateItems(onUpdate))
  ...
  // reducer
  // -------------------------
  renderItems({renderStartPosition, items}) {
  ...
  observeChange() {
    ...
    return this.editor.buffer.onDidChange(event => {
      ...
      if (!isQueryModified(event.newRange, event.oldRange)) {
        this.setModifiedState(false) // Item area modified, direct editor
        return
      }
  ...
  vmpIsInsertMode() {
    this.vmpIsEnabled() && this.editorElement.classList.contains("insert-mode")
  }

  vmpIsNormalMode() {
    this.vmpIsEnabled() && this.editorElement.classList.contains("normal-mode")
  }

  vmpIsEnabled() {
    this.editorElement.classList.contains("vim-mode-plus")
  }
