...
class Ui {
  ...
  async open ({focus = true} = {}) {
    const location = lastOpenedLocation || this.getDefaultLocation()
    if (location === 'center') {
      ...
      atom.workspace.paneContainerForURI(this.editor.getPath()).show()
    }
    if (focus) {
      this.narrowEditor.activatePane()
    }
    lastOpenedLocation = this.narrowEditor.getLocation()
  ...
  switchUiLocation () {
    const destinationPane =
      this.narrowEditor.getLocation() === 'center'
        ? atom.workspace.getBottomDock().getActivePane()
        : this.getCenterPaneToOpen()
  ...
  async preview () {
    ...
    if (this.drawItemAtUpperMiddleOnPreview) {
      redrawPointAtUpperMiddle(editor, item.point)
    } else {
