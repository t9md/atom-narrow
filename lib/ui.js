...
class Ui {
  ...
  updateSearchOptions() {
    ...
    searchOptions.setSearchTerm(this.getSearchTermFromQuery())
    ...
    this.controlBar.updateElements({
      ...
      searchUseRege: searchOptions.searchUseRege,
    })
  }
  ...
  async refresh({force, selectFirstItem, event = {}} = {}) {
    ...
    this.refreshDisposables.add(this.onDidFinishUpdateItems(onFinish))
    ...
    if (this.items.cachedItems) {
      ...
      if (this.useFirstQueryAsSearchTerm) this.updateSearchOptions()
  ...
  renderItems({renderStartPosition, items, filterSpec}) {
    ...
    if (Number.isInteger(firstVisibleScreenRow)) {
      ...
      const end = start + this.editor.getRowsPerPage()
      if (end > 0) {
        const visibleItems = items.slice(Math.max(start, 0), end)
