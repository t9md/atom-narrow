...
class Ui {
  ...
  async open({pending = false, focus = true} = {}) {
  ...
  renderItems({renderStartPosition, items, filterSpec}) {
    ...
    const firstVisibleRow = this.editor.bufferRowForScreenRow(this.editor.getFirstVisibleScreenRow())
    const visibleItems = sliceVisibleItems(items, firstItemRow, firstVisibleRow, this.editor.getRowsPerPage())
    if (visibleItems.length) {
      this.highlighter.highlightItemsOnNarrowEditor(visibleItems, filterSpec)
    }
    return {renderStartPosition}

    function sliceVisibleItems(items, firstItemRow, firstVisibleRow, rowsPerPage) {
      const start = firstVisibleRow - firstItemRow
      const end = start + rowsPerPage
      if (end > 0) {
        return items.slice(Math.max(start, 0), end)
      } else {
        return []
      }
    }
  ...
  async confirm({keepOpen, flash} = {}) {
    const item = this.items.getSelectedItem()
    if (!item) return

    const editor = await this.provider.confirmed(item)
    if (editor) {
