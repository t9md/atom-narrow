...
class Ui {
  ...
  static unregister (ui) {
    this.uiByEditor.delete(ui.outlet)
    this.updateWorkspaceClassList()
  }

  static register (ui) {
    this.uiByEditor.set(ui.outlet, ui)
    this.updateWorkspaceClassList()
  }
  ...
  registerCommands () {
    return atom.commands.add(this.editor.element, {
      ...
      // because it's directly defined on this.editor.element.
  ...
  scrollToColumnZero () {
    const {row} = this.outlet.getCursorBufferPosition()
    this.outlet.scrollToBufferPosition([row, 0], {center: true})
  }
  ...
  get editor () { return this.outlet } // prettier-ignore
  ...
  constructor (provider, {query = ''} = {}, restoredState) {
    ...
    this.split = this.provider.getConfig('split')
    this.useAdjacentPane = this.provider.getConfig('useAdjacentPane') === 'use'
    ...
    // Setup narrow-editor
    // -------------------------
    const dashName = this.provider.dashName
    this.narrowEditor = new NarrowEditor({
      ...
      title: dashName + '-' + Ui.getNextTitleNumber(),
      classList: ['narrow', 'narrow-editor', dashName],
      defaultLocation: Ui.lastLocationByProviderName[this.provider.name] || this.locationToOpen,
      split: this.split,
      useAdjacentPane: this.useAdjacentPane
    })
    ...
    this.outlet = this.narrowEditor.outlet
    this.outlet.onDidRelocate(() => this.onRelocate())

    this.outlet.link(this.provider.editor)
    this.outlet.onDidDestroy(this.destroy.bind(this))
    this.highlighter = new Highlighter(this)
    this.subscribeHighlightEvent()

    this.outlet.element.onDidChangeScrollTop(scrollTop => {
  ...
  onRelocate () {
    this.outlet.component.measureClientContainerHeight()
  ...
  onDidBecomeActivePaneItem () {
    if (!this.outlet.element.hasAttribute('outlet-hidden-in-center')) {
  ...
  setRelocateUiByTabBarDoubleClick () {
    ...
    if (tabElements.length) {
      const index = pane.items.indexOf(this.outlet)
  ...
  relocate () { this.outlet.relocate() } // prettier-ignore
  toggle () { this.outlet.toggle() } // prettier-ignore
  focus () { this.outlet.focus() } // prettier-ignore
  ...
  startUpdateControlBar () {
    ...
    this.outlet.element.classList.add('hide-cursor')
    return new Disposable(() => {
      this.outlet.element.classList.remove('hide-cursor')
  ...
  async refresh ({force, selectFirstItem, event = {}} = {}) {
    ...
    const oldColumn = this.outlet.getCursorBufferPosition().column
  ...
  moveToSearchedWordAtSelectedItem (searchRegex) {
    ...
    this.outlet.setCursorBufferPosition(point)
  }
  ...
  getVisibleItems () {
    const [startRow, endRow] = this.outlet.element.getVisibleRowRange()
