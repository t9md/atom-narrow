...
class Ui {
  ...
  toggleProtected(event) {
    ...
    this.controlBar.update({protected: this.protected})
  }

  toggleAutoPreview(event) {
    suppressEvent(event)
    this.autoPreview = !this.autoPreview
    this.controlBar.update({autoPreview: this.autoPreview})
  ...
  constructor(provider, {query} = {}, restoredState) {
    ...
    this.editorElement.onDidChangeScrollTop(() => {
      this.highlighter.highlightItems(this.items.getVisibleItems(), this.filterSpec)
    })
  ...
  startUpdateItemCount() {
    const intervalID = setInterval(() => {
      this.controlBar.update({itemCount: this.items.getCount()})
    }, 500)
  ...
  updateControlBarRefreshElement() {
    const updateRefreshRunningElement = () => {
      this.controlBar.update({refresh: true})
    }
  ...
  renderItems({renderStartPosition, items, filterSpec}) {
    ...
    if (visibleItemCount > 0) {
      ...
      this.highlighter.highlightItems(visibleItems, filterSpec)
    }
