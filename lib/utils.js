...
function getFirstCharacterPositionForBufferRow (editor, row) {
  let point
  editor.scanInBufferRange(/\S/, editor.bufferRangeForBufferRow(row), event => (point = event.range.start))
...
function getMemoizedRelativizeFilePath () {
  const cache = {}
  return filePath => {
    if (!(filePath in cache)) cache[filePath] = relativizeFilePath(filePath)
