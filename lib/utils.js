...
function saveEditorState(editor) {
  ...
  const oldFoldStartRows = editor.displayLayer.foldsMarkerLayer
    .findMarkers({})
    .map(m => m.getStartPosition().row)

  return ({activateEditor = true} = {}) => {
...
function getMemoizedRelativizeFilePath() {
  const cache = {}
  return filePath =>
    cache[filePath] != null ? cache[filePath] : (cache[filePath] = relativizeFilePath(filePath))
}
...
function isExcludeFilter(text, negateByEndingExclamation) {
  return (
    (text.length > 1 && text.startsWith("!")) || (negateByEndingExclamation && text.endsWith("!"))
  )
}
...
function scanItemsForFilePath(filePath, regex) {
  return atom.workspace
    .open(filePath, {activateItem: false})
    .then(editor => scanItemsForBuffer(editor.buffer, regex))
}
