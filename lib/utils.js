...
function getVisibleBufferRange(editor) {
  const [startRow, endRow] = editor.element.getVisibleRowRange()
  return new Range(
    [editor.bufferRowForScreenRow(startRow), 0],
    [editor.bufferRowForScreenRow(endRow), Infinity]
  )
}
...
function isExcludeFilter(text, negateByEndingExclamation) {
  ...
  return text.startsWith("!") || (negateByEndingExclamation && text.endsWith("!"))
}
