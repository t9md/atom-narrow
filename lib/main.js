...
module.exports = {
  ...
  narrow (name, options) {
    const Klass = require(`./provider/${name}`)
      ...
      observeVimStates(vimState => {
        ...
        vimState.onDidActivateMode(({mode, submode}) => {
          if (mode === 'insert') {
            ...
            if (ui) ui.itemEditor.setReadOnly(false)
          } else if (mode === 'normal') {
            const ui = Ui.get(vimState.editor)
            if (ui) ui.itemEditor.setReadOnly(true)
          }
        })
      })
    )
  ...
  registerProvider (name, klassOrFilePath) {
    Provider.registerProvider(name, klassOrFilePath)
  },

  provideNarrow () {
    return {
      ProviderBase: Provider, // deprecated
      Provider: Provider,
