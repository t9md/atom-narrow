...
const Provider = require('./composition-provider/provider')
...
module.exports = {
  ...
  registerCommands () {
    ...
    return atom.commands.add('atom-text-editor', {
      ...
      // ==== composition based
      'narrow:x-scan': () => this.startNarrow('scan'),
      'narrow:x-scan-by-current-word': () => this.startNarrow('scan', {queryCurrentWord: true}),

      'narrow:x-symbols': () => this.narrow('symbols'),
      'narrow:x-symbols-by-current-word': () => this.narrow('symbols', {queryCurrentWord: true}),

      // 'narrow:x-project-symbols': () => this.narrow('project-symbols'),
      // 'narrow:x-project-symbols-by-current-word': () => this.narrow('project-symbols', {queryCurrentWord: true}),
      //
      // 'narrow:x-git-diff-all': () => this.narrow('git-diff-all'),
      //
      'narrow:x-fold': () => this.startNarrow('fold'),
      'narrow:x-fold-by-current-word': () => this.startNarrow('fold', {queryCurrentWord: true})
      //
      // 'narrow:x-scan': () => this.narrow('scan'),
      // 'narrow:x-scan-by-current-word': () => this.narrow('scan', {queryCurrentWord: true}),
      // 'narrow:x-search': () => this.narrow('search'),
      // 'narrow:x-search-by-current-word': () => this.narrow('search', {queryCurrentWord: true}),
      // 'narrow:x-search-by-current-word-without-focus': () => this.narrow('search', {queryCurrentWord: true, focus: false}),
      // 'narrow:x-search-current-project': () => this.narrow('search', {currentProject: true}),
      // 'narrow:x-search-current-project-by-current-word': () => this.narrow('search', {currentProject: true, queryCurrentWord: true}),
      //
      // 'narrow:x-atom-scan': () => this.narrow('atom-scan'),
      // 'narrow:x-atom-scan-by-current-word': () => this.narrow('atom-scan', {queryCurrentWord: true})
      // ==== composition based
    })
  },
  ...
  startNarrow (name, options) {
    const Klass = require(`./composition-provider/${name}`)
    new Klass().start(options)
  },
