...
class SearchOptions {
  ...
  getIgnoreCaseValueForSearchTerm(term) {
    ...
    return (
      ...
      (sensitivity === "smartcase" && !/[A-Z]/.test(term))
    )
  }
  ...
  setSearchTerm(searchTerm) {
    ...
    if (searchTerm) {
      ...
      if (this.searchWholeWord && !this.searchWholeWordChangedManually)
        this.searchWholeWord = /\w/.test(searchTerm)

      if (!this.searchIgnoreCaseChangedManually) {
        this.searchIgnoreCase = this.getIgnoreCaseValueForSearchTerm(
          this.searchTerm
        )
      }
    }
    ...
    this.grammarCanHighlight =
      this.searchRegex != null &&
      (!this.searchUseRegex ||
        this.searchTerm === _.escapeRegExp(this.searchTerm))
  }
}
