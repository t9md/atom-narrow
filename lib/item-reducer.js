...
function getProjectNameForFilePath(filePath) {
  return path.basename(atom.project.relativizePath(filePath)[0])
}
...
function injectLineHeader(state) {
  ...
  const maxRow = state.maxRow != null
    ? state.maxRow
    : normalItems.map(toRow).reduce(byMax, 0)
  ...
  for (const item of normalItems) {
    item._lineHeader = getLineHeaderForItem(
      item.point,
      maxLineWidth,
      maxColumnWidth
    )
...
function spliceItemsForFilePath(state) {
  const {cachedNormalItems, spliceFilePath, items} = state
  if (cachedNormalItems && spliceFilePath) {
    return {
      items: replaceOrAppendItemsForFilePath(
        spliceFilePath,
        cachedNormalItems,
        items
      ),
...
function insertProjectHeader(state) {
  ...
  for (let item of state.items) {
    ...
    let projectName =
      item.projectName || getProjectNameForFilePath(item.filePath)
...
function insertFileHeader(state) {
  ...
  for (let item of state.items) {
    ...
    if (!(filePath in fileHeadersInserted)) {
      const header = `## ${atom.project.relativize(filePath)}`
...
function filterFilePath(state) {
  ...
  if (filterSpecForSelectFiles) {
    ...
    items
      ...
      .map(item => (item[filterKey] = relativizeFilePath(item.filePath)))
