...
module.exports = class Items {
  ...
  findRowForNormalOrPromptItem (row, direction) {
    return this.findRowBy(row, direction, row => this.isNormalItemRow(row) || this.ui.isPromptRow(row))
  }

  findRowForNormalItem (row, direction) {
    if (this.hasNormalItem()) return this.findRowBy(row, direction, row => this.isNormalItemRow(row))
  }

  findNormalItemInDirection (fromRow, direction) {
    const row = this.findRowForNormalItem(fromRow, direction)
    if (row != null) return this.getItemForRow(row)
  }
  ...
  findEqualLocationItem (item) {
    return this.getNormalItems().find(({point, filePath}) => {
      point.isEqual(item.point) && filePath === item.filePath
    })
  }
