...
module.exports = class Items {
  ...
  // This method is called multipe times on single query.
  // Since item updated async and multiple time against single query.
  // item._row keep row in narrow-editor. And which is also idex of this.items array.
  addItems (items) {
  ...
  findItem (startRow, {direction, includeStartRow = false}, fn) {
    if (includeStartRow) {
      const item = this.forRow(startRow)
      if (fn(item)) {
        return item
      }
    }
