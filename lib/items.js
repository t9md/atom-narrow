...
class Items {
  ...
  findRowForNormalOrPromptItem(row, direction) {
    const normalRowOrPromptRow = row =>
      this.isNormalItemRow(row) || this.ui.isPromptRow(row)
  ...
  findDifferentFileItem(direction) {
    ...
    const row = this.findRowBy(this.getRowForSelectedItem(), direction, row => {
      return (
        this.isNormalItemRow(row) &&
        this.getItemForRow(row).filePath !== filePath
      )
    })
    return this.getItemForRow(row)
  }
  ...
  findEqualLocationItem(itemToFind) {
    ...
    const isEqualLocation = ({point, filePath}) =>
      point.isEqual(itemToFind.point) && filePath === itemToFind.filePath
  ...
  selectItemInDirection(point, direction) {
    ...
    this.selectItem(
      this.findNormalItemInDirection(this.getRowForItem(item), direction)
    )
  }
}
