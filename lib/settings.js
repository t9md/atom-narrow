...
const globalSettings = {
  ...
  useAdjacentPane: {
    default: 'use',
    enum: ['use', 'never'],
    description: 'Wheter to use adjacent pane to open ui'
  },
  locationToOpen: {
    default: 'bottom',
    enum: ['bottom', 'center'],
...
const ProviderConfigTemplate = {
  split: inheritGlobalEnum('split'),
  useAdjacentPane: inheritGlobalEnum('useAdjacentPane'),
...
const searchFamilyConfigs = {
  ...
  Search: newSearchFamilyConfig({
    ...
    agExtraArgs: {
      default: '',
      description: 'Extra `ag` command line args(e.g. `--follow`)'
    },
    rgExtraArgs: {
      default: '',
      description: 'Extra `rg` command line args(e.g. `--follow`)'
    }
  }),
...
class Settings {
  ...
  forceMigrationOldConfiguration () {
    // TODO: remove a month lator 2018.3.5.
    const scopesToMigrate = [
      '',
      'Scan.',
      'Search.',
      'AtomScan.',
    ...
    for (let scope of scopesToMigrate) {
      const value = this.get(`${scope}directionToOpen`)
      if (value == null) continue
      const [split, openPreference] = value.split(':')
      const useAdjacentPane = openPreference === 'always-new-pane' ? 'never' : 'use'
      this.set(`${scope}split`, split)
      this.set(`${scope}useAdjacentPane`, useAdjacentPane)
      this.delete(`${scope}directionToOpen`)
    }
  }
}
