...
module.exports = class Highlighter {
  ...
  highlightNarrowEditorForItems(items, filterSpec) {
    ...
    this.highlightNarrowEditorForHeader(items)
    this.highlightNarrowEditorForSearchTerm(items)
    if (filterSpec) {
      this.highlightNarrowEditorForIncludeFilter(items, filterSpec.include)
    }
  }

  highlightNarrowEditorForHeader(items) {
    let row = this.ui.items.getRowForItem(items[0]) - 1
    const markerLayer = this.markerLayers.header
    for (const item of items) {
      row++
      if (!item.header) continue
      markerLayer.markBufferRange([[row, 0], [row, Infinity]], {
        invalidate: "inside",
      })
    }
  }
  ...
  highlightNarrowEditorForIncludeFilter(items, filterPatterns) {
    ...
    for (let row = startRow; row <= endRow; row++) {
      if (!this.ui.items.isNormalItemRow(row)) continue
