...
module.exports = class Highlighter {
  ...
  highlightItems(items, filterSpec) {
    ...
    const itemRangeForRow = (row, item) => {
      const {start, end} = item.range
      const headerWidth = item._lineHeader ? item._lineHeader.length : 0
      return new Range([row, start.column + headerWidth], [row, end.column + headerWidth])
    }

    const includeFilters = filterSpec
      ? filterSpec.include.map(re => new RegExp(re.source, re.flags + "g"))
      : null
    ...
    for (const item of items) {
      ...
      if (item.range) {
        mark("searchTerm", itemRangeForRow(row, item))
      }
      if (includeFilters) {
