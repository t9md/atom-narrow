...
module.exports = class Highlighter {
  constructor(ui) {
    ...
    if (this.itemHaveRange) {
      this.subscriptions.add(
        this.ui.onDidRefresh(() => {
          if (!this.ui.grammar.searchRegex) this.highlightNarrowEditor()
          this.refreshAll()
        })
      )
    }
    ...
    this.subscriptions.add(
      this.ui.onDidConfirm(() => this.clearCurrentAndLineMarker())
    )
  ...
  highlightNarrowEditor() {
    ...
    if (this.markerLayerForUi) {
      ...
      this.decorationLayerForUi = editor.decorateMarkerLayer(
        this.markerLayerForUi,
        {type: "highlight", class: "narrow-match-ui"}
      )
    }
    ...
    lines.forEach((line, row) => {
      ...
      this.markerLayerForUi.markBufferRange(
        [[row, range.start.column], [row, range.end.column]],
        {invalidate: "inside"}
      )
    })
  }
  ...
  clear() {
    ...
    this.decorationLayerByEditor.forEach(decorationLayer =>
  ...
  highlightCurrentItem(editor, {range}) {
    ...
    const markers = decorationLayer
      .getMarkerLayer()
      .findMarkers({startBufferRow})
  ...
  flashItem(editor, item) {
    ...
    this.clearFlashTimeoutID = setTimeout(
      this.clearFlashMarker.bind(this),
      1000
    )
  }
}
