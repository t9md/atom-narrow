...
module.exports = class Highlighter {
  constructor(ui) {
    ...
    this.subscriptions.add(
      this.ui.onDidRefresh(() => {
        this.highlightNarrowEditorForSearchTerm()
        this.highlightNarrowEditorForFilterMatch()
      })
    )
  ...
  highlightNarrowEditorForSearchTerm() {
    ...
    for (let row of arrayForRange(visibleRange.start.row, visibleRange.end.row)) {
  ...
  // FIXME, update highlight at onDidChangeScrollTop
  highlightNarrowEditorForFilterMatch() {
    ...
    const scanRange = getVisibleBufferRange(editor)

    for (let regex of this.filterPatterns) {
      const {flags} = regex
      regex = new RegExp(regex.source, flags + "g")
      editor.scanInBufferRange(regex, scanRange, ({range}) => {
