...
module.exports = class Highlighter {
  ...
  initMarkerLayers() {
    ...
    createMarkerLayer("searchTerm", "search-term")
    createMarkerLayer("searchTermForPrompt", "search-term")
    createMarkerLayer("includeFilter", "include-filter")
    createMarkerLayer("includeFilterForPrompt", "include-filter")
    createMarkerLayer("excludeFilterForPrompt", "exclude-filter")
    createMarkerLayer("header", "header")
  }
  ...
  highlightNarrowEditorForItems(items, filterSpec) {
    const mark = (markerLayerName, range) => {
      this.markerLayers[markerLayerName].markBufferRange(range, {invalidate: "inside"})
    }

    let includeFilters
    this.filterSpec = filterSpec // keep it to update onScrollTop change.

    if (filterSpec) {
      includeFilters = filterSpec.include.map(re => new RegExp(re.source, re.flags + "g"))
    }
    ...
    for (const item of items) {
      row++
      if (item.header) {
        mark('header', [[row, 0], [row, Infinity]])
        continue
      }

      if (item.range) {
        const {start, end} = item._lineHeader
          ? item.range.translate([0, item._lineHeader.length])
          : item.range
        mark('searchTerm', [[row, start.column], [row, end.column]])
      }
      if (includeFilters) {
        const lineText = this.ui.editor.lineTextForBufferRow(row)
        for (const regex of includeFilters) {
          ...
          while ((match = regex.exec(lineText))) {
            ...
            mark('includeFilter', [[row, match.index], [row, regex.lastIndex]])
          }
        }
      }
    }
