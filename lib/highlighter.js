...
module.exports = class Highlighter {
  ...
  highlightItemsOnNarrowEditor(items, filterSpec, clearOldMarker = false) {
    const {
      header: headerLayer,
    ...
    if (clearOldMarker) {
      headerLayer.clear()
      searchTermLayer.clear()
      includeFilterLayer.clear()
    }
    ...
    const markerOption = {invalidate: "inside"}
    ...
    for (const item of items) {
      ...
      if (item.header) {
        ...
        headerLayer.markBufferRange(range, markerOption)
        continue
      }
      ...
      if (item.range) {
        const {start, end} = item._lineHeader
          ...
          : item.range
        const range = [[itemRow, start.column], [itemRow, end.column]]
        searchTermLayer.markBufferRange(range, markerOption)
      }
      ...
      if (includeFilters) {
        ...
        for (const regex of includeFilters) {
          ...
          while ((match = regex.exec(lineText))) {
            ...
            includeFilterLayer.markBufferRange(range, markerOption)
          }
        }
      }
    }
