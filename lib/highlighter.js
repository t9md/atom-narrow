...
module.exports = class Highlighter {
  initMarkerLayers() {
    ...
    const searchTermOption = {type: "text", class: "narrow-search-term"}
    const includeFilterOption = {type: "text", class: "narrow-include-filter"}
    const excludeFilterOption = {type: "text", class: "narrow-exclude-filter"}

    this.markerLayers = {}
    this.decorationLayers = {}

    const createDecorationLayer = (key, decorationOption) => {
      const markerLayer = editor.addMarkerLayer()
      const decorationLayer = editor.decorateMarkerLayer(markerLayer, decorationOption)
      this.markerLayers[key] = markerLayer
      this.decorationLayers[key] = decorationLayer
    }

    createDecorationLayer("searchTerm", searchTermOption)
    createDecorationLayer("searchTermForPrompt", searchTermOption)
    createDecorationLayer("includeFilter", includeFilterOption)
    createDecorationLayer("includeFilterForPrompt", includeFilterOption)
    createDecorationLayer("excludeFilterForPrompt", excludeFilterOption)
  }
