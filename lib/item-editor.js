...
class ItemEditor {
  ...
  onDidQueryChange (fn) { return this.emitter.on('did-query-change', fn) } // prettier-ignore
  emitDidQueryChange () { this.emitter.emit('did-query-change') } // prettier-ignore
  ...
  withIgnoreCursorMove (fn) {
    this.ui.withIgnoreCursorMove(fn)
  }

  withIgnoreChange (fn) {
    this.ui.withIgnoreChange(fn)
  }
  ...
  observeCursorMove () {
    return this.editor.onDidChangeCursorPosition(event => {
      if (this.ui.ignoreCursorMove) return
      ...
      if (this.isPromptRow(rowToSelect)) {
        ...
        this.ui.emitDidMoveToPrompt()
      } else {
        this.items.selectItem(itemToSelect)
        if (headerWasSkipped) this.ui.moveToSelectedItem({column: goalColumn})
        if (wasAtPrompt) this.ui.emitDidMoveToItemArea()
  ...
  flashCursorLine () {
    ...
    this.editor.decorateMarker(this.cursorLineFlashMarker, {
      type: 'line',
