...
class ItemEditor {
  ...
  observeCursorMove () {
    return this.editor.onDidChangeCursorPosition(event => {
      ...
      const wasAtPrompt = oldRow === PROMPT_ROW
  ...
  moveToSelectedItem (options) {
    this.moveToItem(this.items.getSelectedItem(), options)
  }

  moveToItem (item, {ignoreCursorMove = true, column} = {}) {
    if (!item) {
      return
    }
    if (column == null) {
  ...
  moveToDifferentFileItem (direction) {
    ...
    this.moveToItem(item, {ignoreCursorMove: false})
  }
