...
module.exports = class ControlBar {
  constructor (ui) {
    ...
    const elementFor = (name, {click, hideIf, selected, tips, command} = {}) => {
      const element = this.element.getElementsByClassName(name)[0]
      if (command) {
        click = () => atom.commands.dispatch(keyBindingTarget, command)
        tips = command
      }
    ...
    this.elements = {
      ...
      inlineGitDiff: elementFor('inline-git-diff', {
        ...
        command: 'narrow-ui:git-diff-all:toggle-inline-git-diff'
      }),
      searchIgnoreCase: elementFor('search-ignore-case', {
        command: 'narrow-ui:toggle-search-ignore-case'
      }),
      searchWholeWord: elementFor('search-whole-word', {
        command: 'narrow-ui:toggle-search-whole-word'
      }),
      searchUseRegex: elementFor('search-use-regex', {
        command: 'narrow-ui:toggle-search-use-regex'
      }),
